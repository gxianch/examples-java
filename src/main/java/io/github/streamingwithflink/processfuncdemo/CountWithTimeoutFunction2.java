 ...
 * long timeFlag = MetricTimeHelper.getTimeStampFlagByStep(current.lastModified, "5");
...
public class CountWithTimeoutFunction2
	...
	/** 由这个处理函数负责维护的状态 */
	private ValueState<CountWithTimestamp2> state;

	// 首先获得由这个处理函数（process function）维护的状态
	// 通过 RuntimeContext 访问Flink的keyed state
	@Override
	public void open(Configuration parameters) throws Exception {
		state = getRuntimeContext().getState(new ValueStateDescriptor<>("myState", CountWithTimestamp2.class));
	}
	...
	public void processElement(Tuple2<String, String> value, Context ctx, Collector<Tuple2<String, Long>> out)
		...
		// 获取当前的计数
		CountWithTimestamp2 current = state.value();
		if (current == null) {
			current = new CountWithTimestamp2();
...
//	触发时间  timestamp=current.lastModified + 60000
	...
	public void onTimer(long timestamp, OnTimerContext ctx, Collector<Tuple2<String, Long>> out) throws Exception {
		...
		// 获取调度此计时器的key的状态
		CountWithTimestamp2 result = state.value();
